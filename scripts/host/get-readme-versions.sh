...
fetch "build-essential" "dpkg -s build-essential 2>/dev/null | awk -F': ' '/^Version:/{print \$2}'"
fetch "cargo" "/usr/local/rust/bin/cargo --version | head -n1 | sed -E 's/^[^0-9]*([0-9]+([.][0-9A-Za-z_-]+)*).*$/\1/'"
fetch "ccze" "ccze --version 2>/dev/null | head -n1 | sed -E 's/^[^0-9]*([0-9]+([.][0-9A-Za-z_-]+)*).*$/\1/'"
...
fetch "curl" "curl --version 2>/dev/null | head -n1 | sed -E 's/^[^0-9]*([0-9]+([.][0-9A-Za-z_-]+)*).*$/\1/'"
fetch "doctest" "doctest --version 2>/dev/null | head -n1 | sed -E 's/^[^0-9]*([0-9]+([.][0-9A-Za-z_-]+)*).*$/\1/'"
fetch "doxygen" "doxygen --version | head -n1 | sed -E 's/^[^0-9]*([0-9]+([.][0-9A-Za-z_-]+)*).*$/\1/'"
...
fetch "git" "git --version | head -n1 | sed -E 's/^[^0-9]*([0-9]+([.][0-9A-Za-z_-]+)*).*$/\1/'"
fetch "graphviz" "dot -V 2>&1 | head -n1 | sed -E 's/^[^0-9]*([0-9]+([.][0-9A-Za-z_-]+)*).*$/\1/'"
fetch "FuzzTest" "[[ -d /opt/fuzztest ]] && cd /opt/fuzztest && git rev-parse --short HEAD || echo 'missing'"
...
fetch "lcov" "lcov --version 2>/dev/null | head -n1 | sed -E 's/^[^0-9]*([0-9]+([.][0-9A-Za-z_-]+)*).*$/\1/'"
...
fetch "nlohmann-json" "grep 'version' /usr/include/nlohmann/json.hpp 2>/dev/null | head -n1 | sed -E 's/.*version ([0-9]+([.][0-9A-Za-z_-]+)*).*$/\1/'"
...
fetch "PC-Lint" "[[ -f /opt/pclp/README ]] && grep 'Version' /opt/pclp/README | head -n1 | sed -E 's/.*Version ([0-9]+[^ ]*( [^ ]*)?).*$/\1/' || echo 'missing'"
...
fetch "rust" "/usr/local/rust/bin/rustc --version | head -n1 | sed -E 's/^[^0-9]*([0-9]+([.][0-9A-Za-z_-]+)*).*$/\1/'"
...
fetch "zip" "zip --version 2>/dev/null | grep 'This is Zip' | sed -E 's/.*Zip ([0-9]+([.][0-9A-Za-z_-]+)*).*$/\1/'"
